...
**1. Rode a migration que falta no Supabase.**
A função RPC `resolve_theft_event(event_id, resolved_by)` usada pelo botão
"marcar como resolvido" ainda não existe no banco atual. Abra o SQL Editor
do seu projeto Supabase e rode o conteúdo de `supabase_missing_rpc.sql`
(ele também adiciona a coluna `resolved_by` em `theft_events`, que faltava).
